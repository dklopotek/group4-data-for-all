...
- BACKED BY: `outputs/img/wind-pollen-tool-full.mp4` (the full ~58 s walkthrough of `phase-6/wind-pollen/`, real recording: opens on the data layers, then the CFD pollen render). The slide autoplays it muted and looped, with controls so you can pause / scrub. A short 14 s pollen-only loop is also kept at `outputs/img/wind-pollen-tool.mp4`. On-screen readout ("Calm S · 2 m/s input · real CFD (Infrared SDK) · 474 Platanus trees") is the viewer's own, not annotated by us. Note: the full clip runs ~58 s, so let it play under your narration or scrub to the pollen section (~from the 45 s mark) if short on time.
- LIKELY Q&A + DEFENSE:
