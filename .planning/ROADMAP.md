...
**Requirements**: BRIEF-01, ROB-01, ROB-02, ROB-03, ROB-04, ROB-05, ROB-06, ROB-07, ROB-08, VAL-01, VAL-02, VAL-03, VAL-04, EXT-01, EXT-02, EXT-03
**Success Criteria** (what must be TRUE):
  ...
  6. The keystone external-validation test runs per its pre-registration (`phase-5/external-validation-design.md`): `data/processed/gbif_external_target.parquet` exists (per-cell GBIF fungal richness/presence/effort, leakage-checked), and `outputs/phase-5/external_validation_results.md` reports M0-abiotic-null vs M1-+biotic ΔAdjusted-R², partial-F p, presence CV-AUC delta + LR p, VIF, Moran's I, and an explicit PASS/FAIL verdict against the pre-registered criterion — answering whether the data carries mycorrhizal signal beyond the sealed-surface null (EXT-01, EXT-02, EXT-03)
**Plans**: TBD
**Note**: EXT pre-registration (design + `src/external_validation.py`) is committed BEFORE results, per the team's binding pre-registration contract. The verdict is reported either way (PASS rescues a re-weighted biotic claim; FAIL confirms the reframe empirically).
